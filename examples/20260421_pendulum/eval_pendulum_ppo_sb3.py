"""
Evaluate a trained SB3 PPO model (from train_pendulum_ppo_sb3.py).
...
Usage:
    python eval_pendulum_ppo_sb3.py results_ppo_sb3_sparse
    python eval_pendulum_ppo_sb3.py results_ppo_sb3_sparse --x0 3.14 0
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='Evaluate a trained SB3 PPO model.',
    )
    parser.add_argument('results_dir', type=str,
                        help='directory containing final_model.zip '
                             '(e.g., results_ppo_sb3_sparse)')
    ...
    # Expected format: results_ppo_sb3_{dense|sparse}
    parts = os.path.basename(args.results_dir).split('_')
    sparse_reward = 'sparse' in parts

    # Load model
    model = PPO.load(os.path.join(args.results_dir, 'final_model'))
    max_tau = 5.0
    ...
    for i in range(n):
        for j in range(n):
            ...
            with torch.no_grad():
                V[j, i] = model.policy.predict_values(obs_t).item()
